...
This is the written specification the Scope tab and the analyzer decode against.
...
per-channel axes, the Scope tab's channel picker, its trace colours and its hit
...
The waveforms are the Scope tab's plot. AT00's telemetry and AC00 are
...
Two of their numbers are checks rather than readings, and the Scope tab's
...
Set it to `0` and the Scope tab plots against sample index and says so in its
